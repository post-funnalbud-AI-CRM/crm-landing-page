...
import { Play, Pause, Mail, ArrowRight, Menu, Sun, Moon, Zap, Shield, BarChart3 } from 'lucide-react';
...
const NavbarHero: React.FC<NavbarHeroProps> = ({
  brandName = "Trellin",
  heroTitle = "Your AI-Powered Customer Success Analyst",
  heroDescription = "Know who's at risk, how it's going, and what to do—before it's too late. Trellin is an AI-Powered Customer Success Platform for retainer-based businesses.",
  ...
  const getVideoTransform = () => {
    const maxScroll = 300; // Reduced maximum scroll distance for more controlled effect
    const scrollProgress = Math.min(scrollY / maxScroll, 1);
    
    // Scale from 1 to 1.1 (10% larger) - more conservative scaling
    const scale = 1 + (scrollProgress * 0.1);
    
    // Remove rotation to keep it straight
    // const rotate = scrollProgress * 2; // 0 to 2 degrees
    
    // Very minimal parallax effect to prevent overlap
    const translateY = scrollY * 0.05;
    
    return {
      transform: `scale(${scale}) translateY(${translateY}px)`,
      opacity: 1 - (scrollProgress * 0.02), // Minimal fade for better visibility
    };
  };
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-white via-gray-50 to-gray-100 dark:from-black dark:via-gray-900 dark:to-gray-800 relative overflow-hidden transition-colors duration-300">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-blue-400/10 dark:bg-blue-600/20 rounded-full mix-blend-multiply filter blur-xl opacity-70 animate-blob"></div>
        <div className="absolute top-1/3 right-1/4 w-96 h-96 bg-green-500/10 dark:bg-green-500/20 rounded-full mix-blend-multiply filter blur-xl opacity-70 animate-blob animation-delay-2000"></div>
        <div className="absolute bottom-1/4 left-1/3 w-96 h-96 bg-purple-300/10 dark:bg-purple-700/20 rounded-full mix-blend-multiply filter blur-xl opacity-70 animate-blob animation-delay-4000"></div>
      </div>

      {/* Floating particles effect */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="particle absolute w-2 h-2 bg-blue-400/20 dark:bg-blue-600/30 rounded-full animate-float-1 shadow-sm"></div>
        <div className="particle absolute w-1 h-1 bg-green-500/25 dark:bg-green-500/35 rounded-full animate-float-2"></div>
        <div className="particle absolute w-1.5 h-1.5 bg-purple-300/20 dark:bg-purple-700/30 rounded-full animate-float-3"></div>
        <div className="particle absolute w-1 h-1 bg-blue-600/15 dark:bg-blue-400/25 rounded-full animate-float-4"></div>
        <div className="particle absolute w-2 h-2 bg-green-500/20 dark:bg-green-600/30 rounded-full animate-float-5 shadow-sm"></div>
        <div className="particle absolute w-1 h-1 bg-purple-400/25 dark:bg-purple-500/35 rounded-full animate-float-6"></div>
      </div>

      {/* Subtle animated lines */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none opacity-20 dark:opacity-10">
        <div className="absolute top-1/4 left-0 w-full h-px bg-gradient-to-r from-transparent via-blue-400 dark:via-blue-600 to-transparent animate-slide-right"></div>
        <div className="absolute top-3/4 left-0 w-full h-px bg-gradient-to-r from-transparent via-green-500 dark:via-green-500 to-transparent animate-slide-left"></div>
      </div>
      ...
      <div className="relative z-10">
        <div className="w-full max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
            ...
            <div className="backdrop-blur-md bg-white/80 dark:bg-black/80 border border-gray-200/50 dark:border-gray-700/50 rounded-2xl px-6 py-4 flex items-center justify-between gap-4 shadow-2xl transition-colors duration-300">
              ...
              <div className="flex items-center gap-3">
                <div className="hidden lg:flex items-center gap-3">
                  <a
                    href="#"
                    className="bg-blue-600 dark:bg-blue-500 text-white hover:bg-blue-700 dark:hover:bg-blue-600 py-3 px-6 text-sm rounded-xl font-medium transition-all duration-300 flex items-center gap-2 shadow-lg hover:shadow-xl hover:scale-105"
                  >
                  ...
                  {isMobileMenuOpen && (
                    <ul className="absolute top-full right-0 mt-2 p-2 backdrop-blur-md bg-white/90 dark:bg-black/90 border border-gray-200 dark:border-gray-700 rounded-xl w-56 z-30 animate-in slide-in-from-top-2 duration-200">
                      ...
                      <li className="border-t border-gray-200 dark:border-gray-700 mt-2 pt-2 space-y-2">
                        ...
                        <a
                          href="#"
                          className="w-full bg-blue-600 dark:bg-blue-500 text-white hover:bg-blue-700 dark:hover:bg-blue-600 px-3 py-3 text-sm rounded-lg flex items-center justify-center gap-2 font-medium transition-all duration-300"
                        >
                          {dict.nav.getStarted}<ArrowRight className="h-4 w-4" />
                        </a>
                      </li>
            ...
            <div className="max-w-4xl mx-auto">
              ...
              <div className="mt-10 flex items-center justify-center gap-4 flex-wrap animate-in slide-in-from-bottom-8 duration-1000 delay-400">
                <div className="relative group">
                  <div className="absolute -inset-1 bg-gradient-to-r from-blue-400 to-blue-600 dark:from-blue-600 dark:to-blue-400 rounded-full blur opacity-25 group-hover:opacity-50 transition duration-1000 group-hover:duration-200"></div>
                  <div className="relative backdrop-blur-md bg-white/80 dark:bg-black/80 border border-gray-200 dark:border-gray-700 rounded-full p-1 flex items-center">
                    ...
                    <input 
                      ...
                      className="bg-transparent text-black dark:text-white placeholder-gray-500 dark:placeholder-gray-400 font-medium pl-12 pr-4 py-4 text-base rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 w-80 transition-colors duration-300"
                    />
                  </div>
                </div>
                <button 
                  onClick={handleEmailSubmit} 
                  className="bg-blue-600 dark:bg-blue-500 text-white hover:bg-blue-700 dark:hover:bg-blue-600 px-8 py-4 text-base rounded-full font-medium transition-all duration-300 flex items-center gap-2 shadow-xl hover:shadow-2xl hover:scale-105"
                >
          ...
          {/* --- Enhanced Media Section with Scroll Motion --- */}
          <div 
            ref={videoContainerRef}
            className="relative group animate-in slide-in-from-bottom-12 duration-1000 delay-800 overflow-hidden"
            style={{
              ...getVideoTransform(),
          ...
          >
            <header className="relative w-full aspect-video rounded-3xl overflow-hidden shadow-2xl">
              <Image 
                src={backgroundImage} 
                alt="Trellin Customer Success Dashboard Preview" 
                fill
              ...
              <div className="absolute bottom-6 right-6 z-10">
                {!isVideoPlaying ? (
                  <button 
                    ...
                    className="w-16 h-16 rounded-full bg-white/90 backdrop-blur-md flex items-center justify-center hover:bg-white transition-all duration-300 shadow-2xl hover:scale-110 group"
                  >
                    <Play className="h-8 w-8 text-black fill-current ml-1 group-hover:scale-110 transition-transform duration-300" />
                  </button>
                ) : (
                  <button 
                  ...
                  >
                    {isVideoPaused ? 
                      <Play className="h-8 w-8 text-black fill-current ml-1 group-hover:scale-110 transition-transform duration-300" /> : 
                      <Pause className="h-8 w-8 text-black fill-current group-hover:scale-110 transition-transform duration-300" />
                    }
                  </button>
                )}
              </div>
